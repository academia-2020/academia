from django import forms
from django.contrib.auth.forms import UserCreationForm
from django.contrib.auth.models import User

class RegistroConMail(UserCreationForm):
	email = forms.EmailField(required=True,help_text='Campo requerido y debe ser valida hasta 254 caracteres')

	class Meta:
		model = User
		fields = ('username','email','password1','password2')

	def clean_email(self):
		email = self.cleaned_data['email']
		if User.objects.filter(email=email):
			raise forms.ValidationError('El email ya existe, intente otro')
		return email
		
from .models import perfil
from django.core.exceptions import ValidationError

LISTA_SEXOS = (
		('F','Femenino'),
		('M','Masculino')
	)
class PerfilForm(forms.ModelForm):
	class Meta:
		model= perfil
		fields = ('nombre','avatar','biografia','recibemails','sexo','nacimiento')

		widgets = {
			'nombre': forms.TextInput(attrs={'class':'form-control'}),
			'avatar': forms.ClearableFileInput(attrs={'class':'form-control'}),
			'biografia': forms.Textarea(attrs={'class':'form-control'}),
			'recibemails': forms.CheckboxInput(),
			'sexo':forms.RadioSelect(choices=LISTA_SEXOS),
			'nacimiento': forms.DateInput(attrs={'class':'form-control'},format="%m/%d/%Y")
		}

		labels ={'nombre': 'Nombre','avatar':'Logo','biografia': 'Experiencia','recibemails': 'Puede recibir mails'}

	# def clean_nombre(self):
	# 	nombre = self.cleaned_data['nombre']
	# 	print('nombre',nombre)
	# 	if nombre != 'Hola':
	# 		raise ValidationError('El nombre debe ser Hola')
	# 	return nombre

	# def clean(self):
	# 	data = self.cleaned_data
	# 	nombre = data.get('nombre')
	# 	print('nombres',data)
	# 	biografia = data.get('biografia')
	# 	if nombre != 'Hola' and biografia != 'Como estas':
	# 		raise ValidationError('El nombre debe ser Hola y la biografia Como estas')

	# 	return data

